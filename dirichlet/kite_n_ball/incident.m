...
applyBoundaryCondition(model,'dirichlet','Edge',(5:12),'u',trace_utotal);
...
% no obstacle
    ...
    if nargin == 1
        ...
        dl3 = [0      pi/2   pi       3*pi/2
               pi/2   pi     3*pi/2   2*pi
               3      3      3        3   % region label to left (anticlockwise)
               1      1      1        1]; % region label to right (anticlockwise)
    ...
    x(cbs) = R*cos(s(cbs));
    y(cbs) = R*sin(s(cbs));
    cbs = find(4 < bs & bs <= 8); % Kite shaped
    x(cbs) = 0.7 + 0.3*(cos(s(cbs)) + 0.65*cos(2*(s(cbs))) - 0.65);
    y(cbs) = -0.7 + 0.3*1.5*sin(s(cbs));
    cbs = find(bs > 8); % ball
    x(cbs) = -0.7 + 0.3*cos(s(cbs));
    y(cbs) = 0.7 + 0.3*sin(s(cbs));
    end
