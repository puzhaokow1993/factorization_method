...
a = @(location,state) (location.x.^2+location.y.^2<1).*(-(k^2)-100.*exp(-1./(1+location.x.^2+location.y.^2))) + (location.x.^2+location.y.^2>=1).*(-k^2);
f = @(location,state) complex((location.x.^2+location.y.^2<1).*(100.*(exp(-1./(1+location.x.^2+location.y.^2))).*exp(1i*k*(location.x.*(-cos(theta))+location.y.*(-sin(theta))))));
...
applyBoundaryCondition(model,'dirichlet','Edge',(5:8),'u',trace_utotal);
